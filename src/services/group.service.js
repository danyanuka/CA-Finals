...
async function moveTask(taskToMove, srcGroupId, dstGroupId, board) {
    board = structuredClone(board)
    const srcGroupInx = board.groups.findIndex(group => group.id === srcGroupId)
    const taskSrcIdx = board.groups[srcGroupInx].tasks.findIndex(task => task.id === taskToMove.id)
    board.groups[srcGroupInx].tasks.splice(taskSrcIdx, 1)
    const dstGroupInx = board.groups.findIndex(group => group.id === dstGroupId)
